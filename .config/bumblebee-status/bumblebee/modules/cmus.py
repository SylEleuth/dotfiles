...
class Module(bumblebee.engine.Module):
    def __init__(self, engine, config):
        widgets = [
            ...
            bumblebee.output.Widget(name="cmus.duration", full_text=self.duration),
        ...
        self._fmt = self.parameter("format", "{artist} - {title}")
        self._dur = self.parameter("format", "{position}/{duration}")
        self._status = None
        self._shuffle = False
        self._repeat = False
    ...
    def duration(self, widget):
        return string.Formatter().vformat(self._dur, (), self._tags)
